...
returnsRouter.post('/', authMiddleware, (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    const saleItem = db.prepare('SELECT p.id as product_id, p.type, p.quantity as current_stock, si.quantity as ordered_qty, si.price, si.discount, s.payment_status FROM sale_items si JOIN sales s ON si.sale_id = s.id JOIN products p ON si.product_id = p.id WHERE si.id = ? AND si.sale_id = ?').get(sale_item_id, sale_id) as any;
    ...
    const processReturn = db.transaction(() => {
      ...
      // b. Kembalikan stok
      db.prepare('UPDATE products SET quantity = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?').run(newStockBalance, saleItem.product_id);
...
returnsRouter.get('/', authMiddleware, (req: AuthRequest, res: Response) => {
...
returnsRouter.get('/:id', authMiddleware, (req: AuthRequest, res: Response) => {
  const { id } = req.params;
