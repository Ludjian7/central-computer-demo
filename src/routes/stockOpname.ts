...
stockOpnameRouter.get('/', authMiddleware, (req: AuthRequest, res: Response) => {
  try {
...
stockOpnameRouter.post('/', authMiddleware, roleGuard(['admin', 'owner']), (req: AuthRequest, res: Response) => {
  ...
  try {
    const result = db.transaction(() => {
      ...
      const products = db.prepare("SELECT id, quantity FROM products WHERE type = 'physical' AND is_active = 1").all() as any[];
...
stockOpnameRouter.get('/:id', authMiddleware, (req: AuthRequest, res: Response) => {
  const { id } = req.params;
  try {
    const header = db.prepare(`
      SELECT so.*, u.username as creator_name 
...
stockOpnameRouter.patch('/:id/item', authMiddleware, roleGuard(['admin', 'owner']), (req: AuthRequest, res: Response) => {
  const { id: opnameId } = req.params;
  const { product_id, physical_qty, notes } = req.body;

  try {
    const opname = db.prepare("SELECT status FROM stock_opname WHERE id = ?").get(opnameId) as any;
    if (!opname) {
    ...
    const item = db.prepare("SELECT system_qty FROM stock_opname_items WHERE opname_id = ? AND product_id = ?").get(opnameId, product_id) as any;
    if (!item) {
...
stockOpnameRouter.post('/:id/complete', authMiddleware, roleGuard(['admin', 'owner']), (req: AuthRequest, res: Response) => {
  const { id } = req.params;
  const userId = req.user?.id;

  try {
    db.transaction(() => {
      const opname = db.prepare("SELECT status FROM stock_opname WHERE id = ?").get(id) as any;
      if (!opname || opname.status === 'completed') {
        throw new Error('Opname tidak ditemukan atau sudah selesai');
      }

      const items = db.prepare("SELECT * FROM stock_opname_items WHERE opname_id = ? AND physical_qty IS NOT NULL").all(id) as any[];

      for (const item of items) {
        if (item.difference === 0) continue;

        // Apply adjustment to products
        db.prepare("UPDATE products SET quantity = physical_qty, updated_at = CURRENT_TIMESTAMP WHERE id = ?")
      ...
      db.prepare("UPDATE stock_opname SET status = 'completed', completed_at = CURRENT_TIMESTAMP WHERE id = ?").run(id);
    })();
